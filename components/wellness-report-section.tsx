...
// @ts-ignore jspdf is provided by runtime and shimmed in types/jspdf.d.ts
...
export function WellnessReportSection({ selectedSensor }: WellnessReportSectionProps) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      // Prepare plain-text content, then render it into a real PDF
      const pdfContent = `AirPulse Wellness Report
Generated: ${new Date().toLocaleString()}
Sensor: ${data.sensor?.name || "N/A"} (${data.sensor?.id || "N/A"})
Time Range: ${getTimeRangeLabel(timeRange)}
...
${Object.entries(data.pollutantHistory)
  .map(
    ([pollutant, values]) =>
      `${pollutant}: Avg ${values.avg}, Max ${values.max}, Min ${values.min}, Trend ${values.trend}`,
  )
  .join("\n")}
...
Report generated by AirPulse AI Health Analytics`
      ...
      const lines = doc.splitTextToSize(pdfContent, maxWidth)
      doc.text(lines, marginLeft, marginTop)

      const filename = `airpulse-wellness-report-${format(new Date(), "yyyy-MM-dd")}.pdf`
      doc.save(filename)
      showSuccessMessage(`${filename} downloaded successfully!`)
    } catch (error) {
